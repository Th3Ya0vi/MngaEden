...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        self.height = self.height + lHeight + MBPADDING;
		
        // Move indicator to make room for the label
        indFrame.origin.y -= (floor(lHeight / 2 + MBPADDING / 2));
        indicator.frame = indFrame;
		
        // Set the label position and dimensions
        CGRect lFrame = CGRectMake(floor((frame.size.width - lWidth) / 2) + xOffset,
                                   floor(indFrame.origin.y + indFrame.size.height + MBPADDING),
                                   lWidth, lHeight);
        label.frame = lFrame;
        ...
        if (nil != self.detailsLabelText) {
            ...
            lFrame.origin.y -= (floor(lHeight / 2 + MBPADDING / 2));
            label.frame = lFrame;
			
            // Set label position and dimensions
            CGRect lFrameD = CGRectMake(floor((frame.size.width - lWidth) / 2) + xOffset,
                                        lFrame.origin.y + lFrame.size.height + MBPADDING, lWidth, lHeight);
